...
app.patch("/food/:id", async (req, res) => {
  try {
    ...
    const { name, type, instructions } = req.body;
    const updateRecipe =
      await sql`UPDATE recipes SET "name" = ${name}, "type"=${type}, "instructions"=${instructions} WHERE recipe_id = ${id} RETURNING *`;
...
app.patch("/ingredients/:recipe_id", async (req, res) => {
  try {
    ...
    const ingredientRecipe =
      await sql`UPDATE ingredients SET "name" = ${name}, "quantity"=${quantity}, "condition"=${condition} WHERE ingredient_id = ${recipe_id} RETURNING *`;
...
app.delete("/ingredients/:id", async (req, res) => {
  try {
    ...
    const deleteIngredients =
      await sql`DELETE FROM ingredients WHERE ingredient_id = ${id} RETURNING *`;
...
app.delete("/food/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deleteFood =
      await sql`DELETE FROM recipes WHERE recipe_id = ${id} RETURNING *`;
